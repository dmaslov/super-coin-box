...
gameState.prototype={create:function(){this.createBackground();this.addGlobalVars();this.initSounds();this.addControls();this.createWorld();this.createPlayer();this.createEnemies();this.createCoin();this.createExplosion();this.displayScore();this.displayLives();this.initMobileControls();},update:function(){this.game.physics.arcade.collide(this.player,this.layer);this.game.physics.arcade.collide(this.enemies,this.layer,this.enemyVsLayer,null,this);this.game.physics.arcade.collide(this.player,this.enemies,this.playerVsEnemy,null,this);this.game.physics.arcade.overlap(this.player,this.coin,this.takeCoin,null,this);this.movePlayer();this.resetCloudsPosition();if(!this.player.inWorld){this.playerDie();}
...
else{this.playerDie(enemy);}},enemyDie:function(enemy){var tweenEnemy=this.game.add.tween(enemy);tweenEnemy.to({alpha:0},400,Phaser.Easing.Linear.None,true);this.time.events.add(300,function(){enemy.kill();tweenEnemy.stop();this.addEnemy();},this);this.player.body.velocity.y=-100;this.enemyKillSound.play();this.updateScore(2);this.updateScoreText();this.game.plugins.screenShake.setup({shakeX:false,shakeY:true});this.game.plugins.screenShake.shake(5);},playerDie:function(enemy){if(!this.player.alive){return;}
...
else{this.player.reset(this.game.world.centerX,this.game.world.centerY);this.updateLives();this.enemies.destroy(false);this.createEnemies();this.coin.kill();this.createCoin();}},this);},createWorld:function(){this.map=this.game.add.tilemap('map');this.map.addTilesetImage('tileset');this.game.plugins.levelsManager.init(this.map);this.layer=this.game.plugins.levelsManager.createLevel(true);},createCoin:function(){var randomPos=this.getCoinRandomPosition();this.coin=this.game.add.sprite(randomPos.x,randomPos.y,'gameElements','coin');this.game.physics.arcade.enable(this.coin);this.coin.anchor.setTo(0.5,0.5);},takeCoin:function(){this.updateScore(5);this.updateScoreText();this.updateCoinPosition();this.coinSound.play();this.game.add.tween(this.player.scale).to({x:1.3,y:1.3},50).to({x:1,y:1},150).start();},getPossibleCoinCoordinates:function(){return[{x:140,y:70},{x:250,y:30},{x:360,y:70},{x:60,y:150},{x:250,y:130},{x:440,y:150},{x:110,y:230},{x:380,y:230},{x:90,y:310},{x:250,y:270},{x:350,y:310},];},getCoinRandomPosition:function(){if(!this.coinPos.length){this.coinPos=this.getPossibleCoinCoordinates();}
return this.coinPos[this.game.rnd.integerInRange(0,this.coinPos.length-1)];},updateCoinPosition:function(){if(!this.coinPos.length){this.coinPos=this.getPossibleCoinCoordinates();}
