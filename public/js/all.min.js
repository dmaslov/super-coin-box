...
(function(Phaser){'use strict';function bootState(){this.gameMinWidth=320;this.gameMinHeight=218;this.gameMaxWidth=1024;this.gameMaxHeight=696;}
bootState.prototype={preload:function(){this.game.load.image('progressBar','assets/images/progressBar.png');},create:function(){this.game.stage.backgroundColor='#3498db';this.game.physics.startSystem(Phaser.Physics.ARCADE);if(!this.game.device.desktop){this.game.scale.scaleMode=Phaser.ScaleManager.SHOW_ALL;this.game.scale.minWidth=this.gameMinWidth;this.game.scale.minHeight=this.gameMinHeight;this.game.scale.maxWidth=this.gameMaxWidth;this.game.scale.maxHeight=this.gameMaxHeight;}
else{window.onresize=function(){this.windowResizeHandler();}.bind(this);}
this.game.scale.pageAlignHorizontally=true;this.game.scale.pageAlignVertically=true;this.game.scale.setScreenSize(true);this.game.state.start('load');},windowResizeHandler:function(){this.game.scale.scaleMode=Phaser.ScaleManager.SHOW_ALL;if(window.innerWidth===screen.width){this.game.scale.minWidth=this.gameMinWidth;this.game.scale.minHeight=this.gameMinHeight;this.game.scale.maxWidth=screen.width;this.game.scale.maxHeight=screen.height-24;}
else{this.game.scale.minWidth=this.gameMinWidth;this.game.scale.minHeight=this.gameMinHeight;this.game.scale.maxWidth=this.game.width;this.game.scale.maxHeight=this.game.height;}
this.game.scale.pageAlignHorizontally=true;this.game.scale.pageAlignVertically=true;this.game.scale.setScreenSize(true);}};window['super-coin-box']=window['super-coin-box']||{};window['super-coin-box'].bootState=bootState;})(Phaser);(function(Phaser){'use strict';function gameState(){}
gameState.prototype={create:function(){this.createBackground();this.addGlobalVars();this.initSounds();this.addControls();this.createWorld();this.createPlayer();this.createEnemies();this.createCoin();this.createExplosion();this.displayScore();this.displayLives();this.initMobileControls();},update:function(){this.game.physics.arcade.collide(this.player,this.layer);this.game.physics.arcade.collide(this.enemies,this.layer,this.enemyVsLayer,null,this);this.game.physics.arcade.collide(this.player,this.enemies,this.playerVsEnemy,null,this);this.game.physics.arcade.overlap(this.player,this.coin,this.takeCoin,null,this);this.movePlayer();this.enemyVsNothing();this.resetCloudsPosition();if(!this.player.inWorld){this.playerDie();}
if(this.player.body.x<1||this.player.body.x>this.game.world.width-20){this.playerDie();}
if(this.nextEnemy<this.game.time.now){var start=4000;var end=1000;var score=100;var delay=Math.max(start-(start-end)*this.game.global.score/score,end);this.addEnemy();this.nextEnemy=this.game.time.now+delay;}},createPlayer:function(){this.player=this.game.add.sprite(this.game.world.centerX,this.game.world.centerY,'gameElements','player-01');this.game.physics.arcade.enable(this.player);this.player.anchor.setTo(0.5,0.5);this.player.body.gravity.y=500;this.player.animations.add('right',['player-02','player-03'],8,true);this.player.animations.add('left',['player-04','player-05'],8,true);},movePlayer:function(){if(this.cursor.left.isDown||this.altKeys.left.isDown||this.moveLeft){this.player.body.velocity.x=-200;this.player.animations.play('left');}
else if(this.cursor.right.isDown||this.altKeys.right.isDown||this.moveRight){this.player.body.velocity.x=200;this.player.animations.play('right');}
else{this.player.body.velocity.x=0;this.player.animations.stop();this.player.frameName='player-01';}
if(this.cursor.up.isDown||this.altKeys.up.isDown){this.jumpPlayer();}},jumpPlayer:function(){if(this.player.body.onFloor()){this.player.body.velocity.y=-300;this.jumpSound.play();}},playerVsEnemy:function(player,enemy){if(player.body.touching.down&&enemy.body.touching.up){this.enemyDie(enemy);}
else{this.playerDie(enemy);}},respawnEnemy:function(enemy){enemy.kill();this.addEnemy();},enemyDie:function(enemy){var tweenEnemy=this.game.add.tween(enemy);tweenEnemy.to({alpha:0},400,Phaser.Easing.Linear.None,true);this.time.events.add(300,function(){tweenEnemy.stop();this.respawnEnemy(enemy);},this);this.player.body.velocity.y=-100;this.enemyKillSound.play();this.updateScore(2);this.updateScoreText();this.game.plugins.screenShake.setup({shakeX:false,shakeY:true});this.game.plugins.screenShake.shake(5);},playerDie:function(enemy){if(!this.player.alive){return;}
if(typeof enemy!=='undefined'){this.enemyStop(enemy);}
this.player.kill();this.game.global.playerLives-=1;this.deadSound.play();this.jumpSound.stop();this.startExplosion();this.game.plugins.screenShake.setup({shakeX:true,shakeY:true});this.game.plugins.screenShake.shake(10);this.time.events.add(1000,function(){if(this.game.global.playerLives<1){this.game.state.start('gameOver');}
else{this.player.reset(this.game.world.centerX,this.game.world.centerY);this.updateLives();this.enemies.destroy(false);this.createEnemies();this.respawnCoin();}},this);},createWorld:function(){this.map=this.game.add.tilemap('map');this.map.addTilesetImage('tileset');this.game.plugins.levelsManager.init(this.map);this.layer=this.game.plugins.levelsManager.createLevel(true);},createCoin:function(){var randomPos=this.getCoinRandomPosition();this.coin=this.game.add.sprite(randomPos.x,randomPos.y,'gameElements','coin');this.game.physics.arcade.enable(this.coin);this.coin.anchor.setTo(0.5,0.5);},respawnCoin:function(){this.coin.kill();this.createCoin();},takeCoin:function(){this.updateScore(5);this.updateScoreText();this.updateCoinPosition();this.coinSound.play();this.game.add.tween(this.player.scale).to({x:1.3,y:1.3},50).to({x:1,y:1},150).start();},getPossibleCoinCoordinates:function(){return{'1':[{x:140,y:70},{x:250,y:30},{x:360,y:70},{x:60,y:150},{x:250,y:130},{x:440,y:150},{x:110,y:230},{x:380,y:230},{x:90,y:310},{x:250,y:270},{x:350,y:310},],'2':[{x:130,y:50},{x:250,y:70},{x:370,y:50},{x:30,y:70},{x:190,y:90},{x:310,y:90},{x:470,y:70},{x:30,y:130},{x:150,y:150},{x:210,y:170},{x:290,y:170},{x:350,y:150},{x:450,y:130},{x:30,y:190},{x:70,y:230},{x:110,y:250},{x:190,y:230},{x:310,y:230},{x:430,y:230},{x:470,y:190},{x:50,y:310},{x:170,y:310},{x:350,y:310},{x:450,y:310},],'3':[{x:50,y:50},{x:110,y:70},{x:250,y:70},{x:390,y:70},{x:450,y:50},{x:30,y:150},{x:150,y:150},{x:210,y:110},{x:290,y:110},{x:250,y:150},{x:390,y:150},{x:470,y:150},{x:50,y:210},{x:90,y:190},{x:190,y:230},{x:310,y:230},{x:430,y:190},{x:450,y:210},{x:30,y:270},{x:30,y:310},{x:110,y:310},{x:390,y:310},{x:470,y:270},],'4':[{x:70,y:50},{x:170,y:50},{x:370,y:50},{x:430,y:50},{x:30,y:110},{x:130,y:130},{x:250,y:90},{x:370,y:130},{x:470,y:110},{x:30,y:170},{x:210,y:170},{x:390,y:130},{x:470,y:170},{x:10,y:250},{x:130,y:230},{x:250,y:270},{x:370,y:230},{x:490,y:250},{x:30,y:310},{x:130,y:310},{x:370,y:310},{x:470,y:310},],'5':[{x:10,y:130},{x:90,y:70},{x:210,y:70},{x:310,y:70},{x:430,y:70},{x:490,y:130},{x:250,y:130},{x:150,y:130},{x:350,y:130},{x:90,y:190},{x:310,y:190},{x:430,y:190},{x:10,y:250},{x:150,y:250},{x:250,y:250},{x:350,y:250},{x:490,y:250},{x:10,y:310},{x:150,y:310},{x:350,y:310},{x:490,y:310},]};},getCoinRandomPosition:function(){this.coinPos=this.getPossibleCoinCoordinates()[this.game.global.currentLevel];return this.coinPos[this.game.rnd.integerInRange(0,this.coinPos.length-1)];},updateCoinPosition:function(){if(!this.coinPos.length){this.coinPos=this.getPossibleCoinCoordinates()[this.game.global.currentLevel];}
for(var i=0;i<this.coinPos.length;i++){if(this.coinPos[i].x===this.coin.x){this.coinPos.splice(i,1);}}
var newPos=this.getCoinRandomPosition();this.coin.reset(newPos.x,newPos.y);this.coin.scale.setTo(0,0);this.game.add.tween(this.coin.scale).to({x:1,y:1},300).start();},createEnemies:function(){this.enemies=this.game.add.group();this.enemies.enableBody=true;this.enemies.createMultiple(10,'gameElements','enemy-01');},addEnemy:function(){var enemy=this.enemies.getFirstDead();if(!enemy){return;}
enemy.alpha=1;enemy.anchor.setTo(0.5,1);enemy.reset(this.game.world.centerX,0);enemy.body.gravity.y=500;enemy.body.velocity.x=100*Phaser.Math.randomSign();enemy.body.bounce.x=1;enemy.checkWorldBounds=true;enemy.outOfBoundsKill=true;enemy.animations.add('right',['enemy-02','enemy-03'],8,true);enemy.animations.add('left',['enemy-04','enemy-05'],8,true);if(enemy.body.velocity.x>0){enemy.animations.play('left');}
else if(enemy.body.velocity.x<0){enemy.animations.play('right');}
else{enemy.animations.stop();enemy.frameName='enemy-01';}
this.time.events.repeat(this.game.rnd.integerInRange(2000,6000),9999,function(){enemy.body.velocity.y=-300;if(enemy.body.y<0){this.respawnEnemy(enemy);}},this);},enemyVsLayer:function(enemy,layer){if(enemy.body.velocity.x>0){enemy.animations.play('right');}
else{enemy.animations.play('left');}},enemyVsNothing:function(){this.enemies.forEach(function(enemy){if(enemy.alive){if(enemy.body.x<1||enemy.body.x>this.game.world.width-20){if(enemy.body.x<1){enemy.body.x=1;}
else if(enemy.body.x>this.game.world.width-20){enemy.body.x=this.game.world.width-20;}
enemy.body.velocity.x=-enemy.body.velocity.x;}
if(!enemy.inWorld||enemy.body.y>this.game.world.height){enemy.kill();}}}.bind(this));},enemyStop:function(enemy){enemy.body.velocity.x=0;enemy.body.velocity.y=0;enemy.animations.paused=true;enemy.frameName='enemy-01';},createExplosion:function(){this.emitter=this.game.add.emitter(0,0,15);this.emitter.makeParticles('gameElements','pixel');this.emitter.setYSpeed(-150,150);this.emitter.setXSpeed(-150,150);this.emitter.gravity=0;},startExplosion:function(){this.emitter.x=this.player.x;this.emitter.y=this.player.y;this.emitter.start(true,600,null,15);},displayScore:function(){this.scoreLabel=this.game.add.text(30,30,'score: 0',{font:'21px Arcade',fill:'#ffffff'});this.game.global.score=0;},updateScoreText:function(){this.scoreLabel.text='score: '+this.game.global.score;},updateScore:function(value){this.game.global.score+=value;this.game.global.levelScore+=value;this.nextLevel();},nextLevel:function(){if(this.game.global.levelScore>=this.game.global.maxLevelScore){this.game.global.levelScore=0;++this.game.global.currentLevel;this.player.kill();this.enemies.destroy(false);this.game.plugins.levelsManager.levelTransition();this.time.events.add(2600,function(){this.layer=this.game.plugins.levelsManager.createLevel(true);this.player.alpha=1;this.player.reset(this.game.world.centerX,this.game.world.centerY);this.createEnemies();this.respawnCoin();},this);}},displayLives:function(){this.lives=this.game.add.group();for(var i=0;i<this.maxPlayerLives;i++){this.lives.create(420+17*i,30,'gameElements','live');}},updateLives:function(){this.lives.remove(this.lives.children[this.game.global.playerLives]);},initSounds:function(){this.jumpSound=this.game.add.audio('jump');this.coinSound=this.game.add.audio('coin');this.deadSound=this.game.add.audio('dead');this.enemyKillSound=this.game.add.audio('enemyKill');this.jumpSound.volume=0.1;this.coinSound.volume=0.2;this.deadSound.volume=0.3;this.enemyKillSound.volume=1;},addControls:function(){this.cursor=this.game.input.keyboard.createCursorKeys();this.game.input.keyboard.addKeyCapture([Phaser.Keyboard.UP,Phaser.Keyboard.DOWN,Phaser.Keyboard.LEFT,Phaser.Keyboard.RIGHT,Phaser.Keyboard.SPACEBAR,]);this.altKeys={up:this.game.input.keyboard.addKey(Phaser.Keyboard.W),left:this.game.input.keyboard.addKey(Phaser.Keyboard.A),right:this.game.input.keyboard.addKey(Phaser.Keyboard.D)};},addMobileControls:function(){this.moveLeft=false;this.moveRight=false;this.jumpButton=this.game.add.sprite(420,260,'controlsElements','button-jump');this.jumpButton.inputEnabled=true;this.jumpButton.events.onInputDown.add(this.jumpPlayer,this);this.jumpButton.alpha=0.5;this.leftButton=this.game.add.sprite(-20,260,'controlsElements','button-left');this.leftButton.inputEnabled=true;this.leftButton.events.onInputOver.add(function(){this.moveLeft=true;},this);this.leftButton.events.onInputOut.add(function(){this.moveLeft=false;},this);this.leftButton.events.onInputDown.add(function(){this.moveLeft=true;},this);this.leftButton.events.onInputUp.add(function(){this.moveLeft=false;},this);this.leftButton.alpha=0.5;this.rightButton=this.game.add.sprite(50,260,'controlsElements','button-right');this.rightButton.inputEnabled=true;this.rightButton.events.onInputOver.add(function(){this.moveRight=true;},this);this.rightButton.events.onInputOut.add(function(){this.moveRight=false;},this);this.rightButton.events.onInputDown.add(function(){this.moveRight=true;},this);this.rightButton.events.onInputUp.add(function(){this.moveRight=false;},this);this.rightButton.alpha=0.5;},initMobileControls:function(){if(!this.game.device.desktop){this.addMobileControls();}},addGlobalVars:function(){this.game.plugins.screenShake=this.game.plugins.add(Phaser.Plugin.ScreenShake);this.game.plugins.levelsManager=this.game.plugins.add(Phaser.Plugin.levelsManager);this.game.plugins.levelsManager.setup({levels:5,prefix:'level_',collisionMap:{'1':[1,2,3],'2':[1,2,3],'3':[1,2,3],'4':[1,2,3],'5':[1,2,3]}});this.game.global.score=0;this.game.global.playerLives=3;this.game.global.currentLevel=1;this.game.global.levelScore=0;this.maxPlayerLives=3;this.nextEnemy=0;this.coinPos=[];},createBackground:function(){var bitmap=this.game.add.bitmapData(500,340);var gradient=bitmap.context.createLinearGradient(0,0,0,340);gradient.addColorStop(0,'#3498db');gradient.addColorStop(0.7,'#5eaadd');gradient.addColorStop(1,'#87bbdd');bitmap.context.fillStyle=gradient;bitmap.context.fillRect(0,0,500,340);this.game.add.sprite(0,0,bitmap);this.clouds=[this.game.add.sprite(this.game.world.centerX-200,this.game.world.centerY-130,'gameElements','cloud-01'),this.game.add.sprite(this.game.world.centerX+70,this.game.world.centerY-140,'gameElements','cloud-02'),this.game.add.sprite(this.game.world.centerX-70,this.game.world.centerY-100,'gameElements','cloud-03'),];var cloudsCnt=this.clouds.length;for(var i=0;i<cloudsCnt;i++){this.clouds[i].alpha=0.4;this.game.physics.arcade.enable(this.clouds[i]);this.clouds[i].body.velocity.x=50+((i%2)?i*2:i*3);}},resetCloudsPosition:function(){var cloudsCnt=this.clouds.length;for(var i=0;i<cloudsCnt;i++){if(!this.clouds[i].inWorld){this.clouds[i].x=-20;this.clouds[i].body.velocity.x=50+((i%2)?i*2:i*3);}}}};window['super-coin-box']=window['super-coin-box']||{};window['super-coin-box'].gameState=gameState;})(Phaser);(function(Phaser){'use strict';function gameOverState(){}
gameOverState.prototype={create:function(){this.initTitle();this.initScore();if(this.game.device.desktop){var upKey=this.game.input.keyboard.addKey(Phaser.Keyboard.UP);upKey.onDown.addOnce(this.start,this);}
else{this.game.sound.mute=true;this.game.input.onDown.addOnce(this.start,this);}},start:function(){this.game.state.start('menu');},initTitle:function(){var nameLabel=this.game.add.text(this.game.world.centerX,-50,'Game Over',{font:'60px Arcade',fill:'#fff'});nameLabel.anchor.setTo(0.5,0.5);var nameLabelTween=this.game.add.tween(nameLabel);nameLabelTween.to({y:80},1000).easing(Phaser.Easing.Bounce.Out).start();},initScore:function(){this.initBestScore();var startText;if(this.game.device.desktop){startText='press the up arrow key to start';}
else{startText='touch the screen to start';}
var scoreLabel=this.game.add.text(this.game.world.centerX,this.game.world.centerY,'score: '+this.game.global.score+'\nbest score: '+this.bestScoreValue,{font:'25px Arcade',fill:'#fff',align:'center'});scoreLabel.anchor.setTo(0.5,0.5);var startLabel=this.game.add.text(this.game.world.centerX,this.game.world.height-110,startText,{font:'25px Arcade',fill:'#fff'});startLabel.anchor.setTo(0.5,0.5);startLabel.alpha=0;this.game.add.tween(startLabel).to({alpha:1},1000,Phaser.Easing.Linear.None,true,0,1000,true);},initBestScore:function(){var storagePrefix='super-coin-box::';var bestScore=storagePrefix+'bestScore';this.bestScoreValue=localStorage.getItem(bestScore);if(!this.bestScoreValue){localStorage.setItem(bestScore,0);this.bestScoreValue=0;}
if(this.game.global.score>this.bestScoreValue){localStorage.setItem(bestScore,this.game.global.score);this.bestScoreValue=this.game.global.score;}},createBackground:function(){var bitmap=this.game.add.bitmapData(500,340);var gradient=bitmap.context.createLinearGradient(0,0,0,340);gradient.addColorStop(0,'#3498db');gradient.addColorStop(0.7,'#5eaadd');gradient.addColorStop(1,'#87bbdd');bitmap.context.fillStyle=gradient;bitmap.context.fillRect(0,0,500,340);this.game.add.sprite(0,0,bitmap);}};window['super-coin-box']=window['super-coin-box']||{};window['super-coin-box'].gameOverState=gameOverState;})(Phaser);(function(Phaser){'use strict';Phaser.Plugin.levelsManager=function(game,parent){Phaser.Plugin.call(this,game,parent);this._settings={level:1,levels:99,prefix:'level_',collisionMap:{}};this.map=undefined;this.layer=undefined;};Phaser.Plugin.levelsManager.prototype=Object.create(Phaser.Plugin.prototype);Phaser.Plugin.levelsManager.prototype.constructor=Phaser.Plugin.levelsManager;Phaser.Plugin.levelsManager.prototype.setup=function(obj){this._settings=Phaser.Utils.extend(false,this._settings,obj);};Phaser.Plugin.levelsManager.prototype.init=function(map){this.map=map;};Phaser.Plugin.levelsManager.prototype.levelTransition=function(){if(++this._settings.level>this._settings.levels){this.gameOver();}
else{this.onChangeLevel();}};Phaser.Plugin.levelsManager.prototype.onChangeLevel=function(){var bitmap=this.game.add.bitmapData(this.game.width,this.game.height);bitmap.context.fillStyle='#87bbdd';bitmap.context.fillRect(0,0,this.game.width,this.game.height);this.cover=this.game.add.sprite(0,0,bitmap);this.cover.alpha=0;this.cover.bringToTop();this.levelLabel=this.game.add.text(this.game.world.centerX,this.game.world.height-210,'Level '+this._settings.level,{font:'60px Arcade',fill:'#fff'});this.levelLabel.anchor.setTo(0.5,0.5);this.levelLabel.alpha=0;this.readyLabel=this.game.add.text(this.game.world.centerX,this.game.world.height-110,'get ready..',{font:'25px Arcade',fill:'#fff'});this.readyLabel.anchor.setTo(0.5,0.5);this.readyLabel.alpha=0;this.game.add.tween(this.cover).to({alpha:1},1000,Phaser.Easing.Linear.OutIn).to({alpha:0},1000,Phaser.Easing.Linear.InOut,false,1500).start();this.game.add.tween(this.levelLabel).to({alpha:1},1000,Phaser.Easing.Linear.OutIn).to({alpha:0},1000,Phaser.Easing.Linear.InOut,false,1000).start();this.game.add.tween(this.readyLabel).to({alpha:1},1000,Phaser.Easing.Linear.OutIn).to({alpha:0},1000,Phaser.Easing.Linear.InOut,false,1000).start();};Phaser.Plugin.levelsManager.prototype.createLevel=function(colides,recalculate){var layerName=this.getLevelName();if(this.layer!==undefined){this.layer.destroy();this.layer=undefined;}
this.layer=this.map.createLayer(layerName);var indexes=this._settings.collisionMap[this._settings.level];this.layer.resizeWorld();this.map.setCollision(indexes,colides,layerName,recalculate);return this.layer;};Phaser.Plugin.levelsManager.prototype.gameOver=function(){this.game.state.start('gameOver');};Phaser.Plugin.levelsManager.prototype.getLevelName=function(){return this._settings.prefix+this._settings.level;};})(Phaser);(function(Phaser){'use strict';function loadState(){}
loadState.prototype={preload:function(){var loadingLabel=this.game.add.text(this.game.world.centerX,150,'loading...',{font:'25px Arcade',fill:'#fff'});loadingLabel.anchor.setTo(0.5,0.5);var progressBar=this.game.add.sprite(this.game.world.centerX,200,'progressBar');progressBar.anchor.setTo(0.5,0.5);this.game.load.setPreloadSprite(progressBar);this.game.load.atlas('gameElements','assets/images/game-spritesheet.png','assets/images/game-sprites.json');this.game.load.image('tileset','assets/images/tileset.png');this.game.load.tilemap('map','assets/images/map.json',null,Phaser.Tilemap.TILED_JSON);this.game.load.audio('jump',['assets/sounds/jump.ogg','assets/sounds/jump.mp3']);this.game.load.audio('coin',['assets/sounds/coin.ogg','assets/sounds/coin.mp3']);this.game.load.audio('dead',['assets/sounds/dead.ogg','assets/sounds/dead.mp3']);this.game.load.audio('enemyKill',['assets/sounds/kill_the_enemy.ogg','assets/sounds/kill_the_enemy.mp3']);if(!this.game.device.desktop){this.game.load.atlas('controlsElements','assets/images/controls-spritesheet.png','assets/images/controls-sprites.json');}
else{this.game.load.audio('intro',['assets/sounds/intro.ogg','assets/sounds/intro.mp3']);}},create:function(){this.game.state.start('menu');}};window['super-coin-box']=window['super-coin-box']||{};window['super-coin-box'].loadState=loadState;})(Phaser);window.onload=function(){'use strict';var states=window['super-coin-box'],game=new Phaser.Game(500,340,Phaser.AUTO,'game');game.global={score:0,levelScore:0,maxLevelScore:50,playerLives:3,currentLevel:1};game.state.add('boot',states.bootState);game.state.add('load',states.loadState);game.state.add('menu',states.menuState);game.state.add('game',states.gameState);game.state.add('gameOver',states.gameOverState);game.state.start('boot');};(function(Phaser){'use strict';function menuState(){}
menuState.prototype={create:function(){this.createMoovie();this.initMuteButton();this.initTitle();this.initScore();this.playMusic();if(this.game.device.desktop){var upKey=this.game.input.keyboard.addKey(Phaser.Keyboard.UP);upKey.onDown.addOnce(this.start,this);}
else{this.game.sound.mute=true;this.game.input.onDown.addOnce(this.start,this);}},update:function(){this.game.physics.arcade.collide(this.introPlayer,this.introLayer);this.game.physics.arcade.collide(this.introCoin,this.introLayer);this.game.physics.arcade.overlap(this.introPlayer,this.introCoin,this.grabCoin,null,this);this.playMoovie();},start:function(){this.stopMusic();this.game.state.start('game');},initMuteButton:function(){if(!this.game.device.desktop){return;}
this.muteButton=this.game.add.button(20,20,'gameElements',this.toggleSound,this);this.muteButton.frameName='muteButton-01';this.muteButton.input.useHandCursor=true;var muteKey=this.game.input.keyboard.addKey(Phaser.Keyboard.M);muteKey.onDown.add(this.toggleSound,this);if(this.game.sound.mute){this.muteButton.frameName='muteButton-02';}},toggleSound:function(){this.game.sound.mute=!this.game.sound.mute;this.muteButton.frameName=this.game.sound.mute?'muteButton-02':'muteButton-01';},initTitle:function(){var nameLabel=this.game.add.text(this.game.world.centerX,-50,'Super Coin Box',{font:'60px Arcade',fill:'#fff'});nameLabel.anchor.setTo(0.5,0.5);var nameLabelTween=this.game.add.tween(nameLabel);nameLabelTween.to({y:80},1000).easing(Phaser.Easing.Bounce.Out).start();if(this.game.device.desktop){var muteLabel=this.game.add.text(150,this.game.world.height-10,'press M button or click on the volume icon to toggle sound',{font:'11px Arial',fill:'#fff'});muteLabel.anchor.setTo(0.5,0.5);}},initScore:function(){this.initBestScore();var startText;if(this.game.device.desktop){startText='press the up arrow key to start';}
else{startText='touch the screen to start';}
var scoreLabel=this.game.add.text(this.game.world.centerX,this.game.world.centerY,'score: '+this.game.global.score+'\nbest score: '+this.bestScoreValue,{font:'25px Arcade',fill:'#fff',align:'center'});scoreLabel.anchor.setTo(0.5,0.5);var startLabel=this.game.add.text(this.game.world.centerX,this.game.world.height-110,startText,{font:'25px Arcade',fill:'#fff'});startLabel.anchor.setTo(0.5,0.5);startLabel.alpha=0;this.game.add.tween(startLabel).to({alpha:1},1000,Phaser.Easing.Linear.None,true,0,1000,true);},initBestScore:function(){var storagePrefix='super-coin-box::';var bestScore=storagePrefix+'bestScore';this.bestScoreValue=localStorage.getItem(bestScore);if(!this.bestScoreValue){localStorage.setItem(bestScore,0);this.bestScoreValue=0;}
if(this.game.global.score>this.bestScoreValue){localStorage.setItem(bestScore,this.game.global.score);this.bestScoreValue=this.game.global.score;}},playMusic:function(){if(!this.game.device.desktop){return;}
this.introMusic=this.game.add.audio('intro',1,true);this.introMusic.volume=0.5;this.introMusic.loop=true;this.introMusic.play();},stopMusic:function(){if(this.introMusic){this.introMusic.stop();}},createBackground:function(){var bitmap=this.game.add.bitmapData(500,340);var gradient=bitmap.context.createLinearGradient(0,0,0,340);gradient.addColorStop(0,'#3498db');gradient.addColorStop(0.7,'#5eaadd');gradient.addColorStop(1,'#87bbdd');bitmap.context.fillStyle=gradient;bitmap.context.fillRect(0,0,500,340);this.game.add.sprite(0,0,bitmap);},createMoovie:function(){this.createBackground();this.introPlayer=this.game.add.sprite(this.game.world.centerX,this.game.world.centerY,'gameElements','player-01');this.game.physics.arcade.enable(this.introPlayer);this.introPlayer.alpha=1;this.introPlayer.anchor.setTo(0.5,1);this.introPlayer.body.gravity.y=500;this.introPlayer.body.velocity.x=100*Phaser.Math.randomSign();this.introPlayer.body.bounce.x=1;this.introPlayer.checkWorldBounds=true;this.introPlayer.outOfBoundsKill=true;this.introPlayer.animations.add('right',['player-02','player-03'],8,true);this.introPlayer.animations.add('left',['player-04','player-05'],8,true);this.time.events.repeat(2000,9999,function(){this.introPlayer.body.velocity.y=-200;},this);this.time.events.add(2500,function(){this.introCoin=this.game.add.sprite(this.game.world.centerX+this.getRandomX(),80,'gameElements','coin');this.game.physics.arcade.enable(this.introCoin);this.introCoin.body.gravity.y=500;this.introCoin.anchor.setTo(1,0.5);this.introCoin.checkWorldBounds=true;this.introCoin.outOfBoundsKill=true;},this);var introMap=this.game.add.tilemap('map');introMap.addTilesetImage('tileset');this.introLayer=introMap.createLayer('menu');this.introLayer.resizeWorld();introMap.setCollision([1,3],true,'menu');this.clouds=[this.game.add.sprite(this.game.world.centerX-200,this.game.world.centerY-110,'gameElements','cloud-01'),this.game.add.sprite(this.game.world.centerX+70,this.game.world.centerY-120,'gameElements','cloud-02'),this.game.add.sprite(this.game.world.centerX-70,this.game.world.centerY-80,'gameElements','cloud-03'),];var cloudsCnt=this.clouds.length;for(var i=0;i<cloudsCnt;i++){this.clouds[i].alpha=0.4;this.game.physics.arcade.enable(this.clouds[i]);this.clouds[i].body.velocity.x=50+((i%2)?i*2:i*3);}},playMoovie:function(){if(this.introPlayer.body.velocity.x>0){this.introPlayer.animations.play('right');}
else if(this.introPlayer.body.velocity.x<0){this.introPlayer.animations.play('left');}
else{this.introPlayer.animations.stop();this.introPlayer.frameName='player-01';}
var cloudsCnt=this.clouds.length;for(var i=0;i<cloudsCnt;i++){if(!this.clouds[i].inWorld){this.clouds[i].x=-20;this.clouds[i].body.velocity.x=50+((i%2)?i*2:i*3);}}},grabCoin:function(){this.introCoin.reset(this.game.world.centerX+this.getRandomX(),80);this.game.add.tween(this.introPlayer.scale).to({x:1.3,y:1.3},50).to({x:1,y:1},150).start();},getRandomX:function(){return this.game.rnd.integerInRange(0,130)*Phaser.Math.randomSign();}};window['super-coin-box']=window['super-coin-box']||{};window['super-coin-box'].menuState=menuState;})(Phaser);(function(Phaser){'use strict';Phaser.Plugin.ScreenShake=function(game,parent){Phaser.Plugin.call(this,game,parent);this._settings={shakesCount:0,shakeX:true,shakeY:true,sensCoef:0.5};this.game.camera.bounds=null;this._moveCamera=function(){if(this._settings.shakesCount>0){var sens=this._settings.shakesCount*this._settings.sensCoef;if(this._settings.shakesCount%2){this.game.camera.x+=this._settings.shakeX?sens:0;this.game.camera.y+=this._settings.shakeY?sens:0;}
else{this.game.camera.x-=this._settings.shakeX?sens:0;this.game.camera.y-=this._settings.shakeY?sens:0;}
this._settings.shakesCount--;if(this._settings.shakesCount===0){this.game.camera.setPosition(0,0);}}};};Phaser.Plugin.ScreenShake.prototype=Object.create(Phaser.Plugin.prototype);Phaser.Plugin.ScreenShake.prototype.constructor=Phaser.Plugin.ScreenShake;Phaser.Plugin.ScreenShake.prototype.setup=function(obj){this._settings=Phaser.Utils.extend(false,this._settings,obj);};Phaser.Plugin.ScreenShake.prototype.shake=function(count){this._settings.shakesCount=count;};Phaser.Plugin.ScreenShake.prototype.update=function(){this._moveCamera();};})(Phaser);