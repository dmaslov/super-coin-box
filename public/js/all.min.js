...
if(this.nextEnemy<this.game.time.now){var start=4000;var end=1000;var score=100;var delay=Math.max(start-(start-end)*this.game.global.score/score,end);this.addEnemy();this.nextEnemy=this.game.time.now+delay;}},createPlayer:function(){this.player=this.game.add.sprite(this.game.world.centerX,this.game.world.centerY,'player');this.game.physics.arcade.enable(this.player);this.player.anchor.setTo(0.5,0.5);this.player.body.gravity.y=500;this.player.animations.add('right',[1,2],8,true);this.player.animations.add('left',[3,4],8,true);},movePlayer:function(){if(this.cursor.left.isDown||this.altKeys.left.isDown||this.moveLeft){this.player.body.velocity.x=-200;this.player.animations.play('left');}
...
else{this.player.body.velocity.x=0;this.player.animations.stop();this.player.frame=0;}
...
this.introMusic=this.game.add.audio('intro',1,true);this.introMusic.volume=0.5;this.introMusic.loop=true;this.introMusic.play();},stopMusic:function(){if(this.introMusic){this.introMusic.stop();}},createBackground:function(){var bitmap=this.game.add.bitmapData(500,340);var gradient=bitmap.context.createLinearGradient(0,0,0,340);gradient.addColorStop(0,'#3498db');gradient.addColorStop(0.7,'#5eaadd');gradient.addColorStop(1,'#87bbdd');bitmap.context.fillStyle=gradient;bitmap.context.fillRect(0,0,500,340);this.game.add.sprite(0,0,bitmap);},createMoovie:function(){this.createBackground();this.introPlayer=this.game.add.sprite(this.game.world.centerX,this.game.world.centerY,'player');this.game.physics.arcade.enable(this.introPlayer);this.introPlayer.alpha=1;this.introPlayer.anchor.setTo(0.5,1);this.introPlayer.body.gravity.y=500;this.introPlayer.body.velocity.x=100*Phaser.Math.randomSign();this.introPlayer.body.bounce.x=1;this.introPlayer.checkWorldBounds=true;this.introPlayer.outOfBoundsKill=true;this.introPlayer.animations.add('right',[1,2],8,true);this.introPlayer.animations.add('left',[3,4],8,true);var introMap=this.game.add.tilemap('intro_map');introMap.addTilesetImage('tileset_intro');this.introLayer=introMap.createLayer('main');this.introLayer.resizeWorld();introMap.setCollision([1,2]);this.clouds=[this.game.add.sprite(this.game.world.centerX-200,this.game.world.centerY-110,'cloud1'),this.game.add.sprite(this.game.world.centerX+70,this.game.world.centerY-120,'cloud2'),this.game.add.sprite(this.game.world.centerX-70,this.game.world.centerY-80,'cloud3'),];var cloudsCnt=this.clouds.length;for(var i=0;i<cloudsCnt;i++){this.clouds[i].alpha=0.4;this.game.physics.arcade.enable(this.clouds[i]);this.clouds[i].body.velocity.x=50+((i%2)?i*2:i*3);}},playMoovie:function(){if(this.introPlayer.body.velocity.x>0){this.introPlayer.animations.play('right');}
else if(this.introPlayer.body.velocity.x<0){this.introPlayer.animations.play('left');}
else{this.introPlayer.animations.stop();this.introPlayer.frame=0;}
var cloudsCnt=this.clouds.length;for(var i=0;i<cloudsCnt;i++){if(!this.clouds[i].inWorld){this.clouds[i].x=-20;this.clouds[i].body.velocity.x=50+((i%2)?i*2:i*3);}}}};window['super-coin-box']=window['super-coin-box']||{};window['super-coin-box'].menuState=menuState;})(Phaser);